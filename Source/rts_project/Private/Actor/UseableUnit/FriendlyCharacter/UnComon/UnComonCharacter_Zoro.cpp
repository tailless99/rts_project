#include "Actor/UseableUnit/FriendlyCharacter/UnComon/UnComonCharacter_Zoro.h"
...
AUnComonCharacter_Zoro::AUnComonCharacter_Zoro()
{
	// 회전 설정
	...
	// 사용 무기 코드 설정
	UseWeaponCode = TEXT("0002");
}

void AUnComonCharacter_Zoro::BeginPlay()
{
	Super::BeginPlay();
...
void AUnComonCharacter_Zoro::SetMovePointLocation()
{
	for (TActorIterator<APlayerUnitTypeASpawnLocation> It(GetWorld()); It; ++It)
	{
		APlayerUnitTypeASpawnLocation* Actor = *It;

		if (FName(Actor->GetActorNameOrLabel()) == TEXT("TypeB_PointA")) {
			StorageStartLocation = Actor->GetActorLocation();
		}
		if (FName(Actor->GetActorNameOrLabel()) == TEXT("TypeB_PointB")) {
...
bool AUnComonCharacter_Zoro::SkillBtn01()
{
...
bool AUnComonCharacter_Zoro::SkillBtn05()
{
	// 초기화
	MixtureUnits.Empty();

	for (TActorIterator<AUnComonCharacter_Zoro> It(GetWorld()); It; ++It)
	{
		AUnComonCharacter_Zoro* Actor = *It;
	...
	if (MixtureUnits.Num() == 3) {
		waveComponent->CallbackRequest_SpawnUnit(FName(TEXT("0005")));

		// 재료 액터들 삭제
		for (AActor* actor : MixtureUnits)
		{
			AUnComonCharacter_Zoro* playerUnit = Cast<AUnComonCharacter_Zoro>(actor);
